...
def play_game():
    global board, starting_player
    board = [[' ' for _ in range(3)] for _ in range(3)]
    current_player = starting_player

    while winner(board) is None:
        if current_player == 'X':
            i, j = agent_move(board, 'X')
        else:
            i, j = agent_move(board, 'O')
...
def agent_move(board, player):
    ...
    next_board[action[0]][action[1]] = player
    reward = 0
    next_state = board_to_string(next_board)
    
    if winner(next_board) == player:
        reward = 1
    elif winner(next_board) and winner(next_board) != player:
        reward = -1
    ...
    updated_q_value = current_q_value + alpha * (reward - current_q_value)
    Q[state][action] = updated_q_value

    # Save the updated Q-value to the database
    save_q_value(state, action, updated_q_value)
    return action
...
def decrease_epsilon():
    global epsilon
    epsilon *= 0.998  # Decrease epsilon by 0.5% each time
...
load_all_q_values()
for _ in range(1000):  # Play 100 games
    play_game()
    decrease_epsilon()
